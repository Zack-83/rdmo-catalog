...
Dateiname <- "PRONTO/rdmo-ptb-options.xml"
...
Übersetzung <- DE %>% full_join(EN,"Pfad") %>% full_join(FR,"Pfad") %>% full_join(IT,"Pfad")
...
fix(Übersetzung)
write_tsv(Übersetzung,"Traduzione00.tsv",na="")

##########################
# aggiornamento traduzione

Übersetzung <- read_tsv("../Traduzione_catalogo.tsv")
Übersetzung <- read_tsv("Traduzione0.tsv")
